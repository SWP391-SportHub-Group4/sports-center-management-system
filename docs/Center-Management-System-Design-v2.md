...
Theo `00-Source-of-Truth.md`, SSOT có ưu tiên cao nhất. File `SportManagement_BusinessRules_v1.2.docx` và bản văn bản [Business-Rules.md](Business-Rules.md) quản lý Business Rules chi tiết; sáu mục được phân loại lại dùng nguồn tại SSOT §0.1. Tài liệu Design chỉ tham chiếu yêu cầu, không ghi đè SSOT hoặc tự chốt entity/enum/state. Bảng dưới đây là mapping lịch sử (để hiểu vì sao số ID không liên tục), không phải danh sách việc cần làm.
...
| Original BR-06 | **BR-2**, **BR-32**, **BR-39** | Bản hiện tại: Administrator tạo tài khoản/gán vai trò; Manager xem báo cáo và cấu hình nghiệp vụ |
...
### 0.1 Phân loại lại BR thành NFR và yêu cầu chức năng

Ngày 11/09/2026: BR-34–38 và BR-48 ngừng dùng như Business Rules. Nguồn nội dung hiện tại: [Non-Functional-Requirements.md](Non-Functional-Requirements.md), theo ủy quyền SSOT §0.1. Các nhắc tới mã cũ ở phần lịch sử phía trên chỉ phục vụ truy vết.

| Mã cũ | Tham chiếu hiện tại |
|---|---|
| BR-34 | NFR-NOTIFY-01; ARCH-NOTIFY-01 tại §6 |
| BR-35 | NFR-PERF-API-01 |
| BR-36 | NFR-AVAIL-01 |
| BR-37 | NFR-BACKUP-01 |
| BR-38 | NFR-SEC-01; nhất quán với BR-5 |
| BR-48 | NFR-PERF-REPORT-01; FR-REPORT-RETRY-01 |

Các điều kiện đo còn thiếu nằm trong tài liệu NFR và SSOT §7. Việc đồng bộ tham chiếu không xác nhận các đề xuất ERD/state bên dưới đã được SSOT chốt; mọi khác biệt vẫn theo SSOT §2–7.

### 0.2 Đồng bộ BR ngày 11/09/2026

Quyết định hiện hành theo SSOT §0.2: năm vai trò; BR-50 cố định chính sách tại xác nhận đăng ký; BR-20/21/53 không ghi No-show cho đăng ký đã hủy hoặc ghi đè Attendance; BR-54 hủy đăng ký/hoàn lượt và tự đăng ký lại; BR-55 hạn hai tháng hoặc 12 tháng từ cọc đầu tiên.

Các API và schema dưới đây là **thiết kế đề xuất**, chỉ được triển khai khi field/enum tương ứng được SSOT chốt. Các trạng thái viết hoa trong sơ đồ là ký hiệu thiết kế; enum nguồn duy nhất ở SSOT §3. Thuật ngữ `CANCELLED_BY_CENTER` trong sơ đồ cũng chỉ là nhãn đề xuất, chưa phải enum mới đã duyệt.
...
=======
    USERS {
        ...
        string Phone
        uuid RoleID FK
        string Status "khoa/mo khoa theo BR-6; enum cho chot"
        datetime CreatedAt
    }
    ROLES {
        uuid RoleID PK
        string RoleName "UserRole theo SSOT section 3, gom SystemAdministrator"
>>>>>>> Stashed changes
    }
...
=======
    ...
    CLASSES {
        ...
        string Name
        string Discipline
        uuid DefaultRoomID FK
        uuid DefaultCoachID FK "nullable"
        int Capacity
        string Status "ACTIVE, ARCHIVED"
    }
    CLASS_RECURRENCE {
        uuid RecurrenceID PK
        uuid ClassID FK
    ...
    CLASS_SESSIONS {
        uuid SessionID PK
        uuid ClassID FK
        uuid RecurrenceID FK "nullable, null = ad-hoc/rescheduled"
        uuid RoomID FK
        uuid CoachID FK
        datetime StartAtUtc
        datetime EndAtUtc
    ...
    ENROLLMENTS {
        ...
        string Status "enum cho chot, can phan biet huy do trung tam BR-54"
...
=======
- **Payment tách khỏi Invoice**: `INVOICES` (hóa đơn, bất biến) → `INVOICE_ITEMS` (dòng chi tiết) → `PAYMENTS` (từng giao dịch thu tiền, cọc và các lần nộp thêm; không quản lý tín dụng) → `PAYMENT_ADJUSTMENTS` (refund/correction, có workflow duyệt riêng, đúng BR-40/BR-42).
- **Lịch lặp tách khỏi session cụ thể**: `CLASS_RECURRENCE` định nghĩa pattern (ngày trong tuần, giờ, timezone, hiệu lực từ-đến); một job định kỳ sinh `CLASS_SESSIONS` trước N tuần. Dời lịch theo BR-54 kết thúc buổi cũ và tạo buổi thay thế có `RescheduledFromSessionID`; hủy/dời không thay đổi pattern gốc. Hủy đăng ký còn hiệu lực, hoàn lượt đã trừ và thông báo; học viên tự đăng ký lại, không tự chuyển/giữ chỗ.
...
### 2.2 Enrollment và Attendance là hai vòng đời riêng
...
=======
    [*] --> CONFIRMED : Đăng ký theo BR-16/19
    CONFIRMED --> CANCELLED_ON_TIME : Hủy tại hoặc trước deadline BR-50 / hoàn lượt
    CONFIRMED --> CANCELLED_LATE : Hủy trễ / không hoàn lượt
    CONFIRMED --> CANCELLED_BY_CENTER : Manager hủy hoặc dời buổi chưa bắt đầu / hoàn lượt BR-54
    CANCELLED_ON_TIME --> [*]
    CANCELLED_LATE --> [*]
    CANCELLED_BY_CENTER --> [*]
```

Không biến Enrollment thành Attendance. Đăng ký đã hủy được giữ làm lịch sử; `DELETE /enrollments/...` không xóa vật lý. Hạn cuối hủy cá nhân sau khi buổi bắt đầu chưa được BR-17 chốt (SSOT BOOK-01).

```mermaid
flowchart TD
    A[Buổi thực tế đã diễn ra và kết thúc] --> B{Đăng ký còn hiệu lực?}
    B -->|Không| X[Không tạo No-show]
    B -->|Có| C{Đã có Attendance?}
    C -->|Có| Y[Giữ nguyên bản ghi]
    C -->|Không| N[Tạo NoShow]
```

Coach được phân công hoặc Receptionist ghi Present/Absent theo BR-22/53. `AttendanceFinalizerJob` chỉ xử lý buổi thực tế đã diễn ra và kết thúc, không quét buổi bị hủy hoặc buổi cũ đã dời chỉ vì đã qua EndAtUtc. Tối đa một Attendance/Member/Session (BR-21), tạo nguyên tử và không ghi đè bản ghi có sẵn. Enum C# theo SSOT: Present, Absent, NoShow; ký hiệu lưu DB cần mapping thống nhất.

### 2.2.1 ClassSession

Xem [ClassSession-diagram.md](ClassSession-diagram.md). BR-54 chốt dời tạo buổi thay thế; các cơ chế chốt COMPLETED hoặc mở lại buổi vẫn chờ SSOT CS-07.
>>>>>>> Stashed changes
...
=======
    ISSUED --> PARTIALLY_PAID: 0 < tổng Payment SUCCESS < TotalAmount
...
**BR-55:** hạn ban đầu hai tháng từ IssuedAt; nhận cọc đầu tiên trong hạn đặt lại hạn đầy đủ thành 12 tháng từ khoản cọc đầu tiên. Các khoản tiếp theo không gia hạn. Field lưu deadline/cọc và quy ước tính tháng chưa chốt; ERD hiện chưa phải schema hoàn chỉnh cho BR-55.

Không suy ra quá hạn tự chuyển VOID/CANCELLED, tự mất cọc hoặc tự cho thanh toán muộn. Các cạnh thanh toán trên chỉ mô tả nhận tiền theo BR-30; trường hợp quá hạn chờ SSOT PAY-01. Công thức số dư và ảnh hưởng điều chỉnh trên state PAID chờ PAY-02; sơ đồ không đủ để triển khai các trường hợp này. MemberPackage hết lượt được hoàn có trở lại Active hay không chờ MEM-01, không coi Expired luôn là trạng thái cuối.
...
## 3. Ràng buộc nghiệp vụ và transaction đề xuất

Các ràng buộc đã chốt cần bảo vệ ở schema/transaction, không chỉ ở API. SQL minh họa phải được đối soát enum/field với SSOT trước khi code; mục thanh toán còn mở theo PAY-02.
...
| 6 | Kiểm soát tổng thu và điều chỉnh (BR-41/43/52) | Chờ chốt công thức ở SSOT PAY-02. Cần khóa Invoice khi ghi thu/hoàn để tránh race, nhưng không triển khai công thức hiện mâu thuẫn về tổng Payment sau hoàn tiền |
>>>>>>> Stashed changes
...
=======
| Nơi cấu hình deadline hủy lớp | Lưu trong bảng cấu hình hệ thống (`SystemSettings` hoặc field `CancellationDeadlineHours` trên `MembershipPackages`/`Classes` nếu muốn cấu hình theo từng loại), do Center Manager chỉnh qua `PUT /api/settings`; gắn chính sách **tại thời điểm xác nhận đăng ký**, không hồi tố khi Manager đổi cấu hình; hủy tại hoặc trước deadline là đúng hạn. Chốt cách lưu phiên bản/snapshot tại SSOT CS-04 trước khi thêm field | BR-50 |
| Công thức refund/adjustment mặc định | BR-52 nêu tỷ lệ lượt/ngày còn lại và Manager được điều chỉnh khi duyệt; chưa chốt giới hạn theo số tiền thực nhận có thể hoàn. Không triển khai trước khi xử lý SSOT PAY-02 | BR-52 |
| Khi nào Attendance = Absent vs No-show | `Absent`: Coach được phân công/Receptionist ghi tay; `NoShow`: job chỉ tạo sau buổi thực tế diễn ra và kết thúc, đăng ký còn hiệu lực, chưa có Attendance; không ghi đè, không áp cho đăng ký đã hủy | BR-53 |
>>>>>>> Stashed changes

---

### 3.2 Hủy hoặc dời buổi theo BR-54

Manager xác nhận hủy/dời một buổi chưa bắt đầu. Trong một transaction: khóa buổi để ngăn đăng ký đồng thời, đánh dấu buổi cũ hủy/đã dời, tạo buổi thay thế nếu dời, hủy các đăng ký còn hiệu lực, hoàn đúng lượt đã trừ một lần và cập nhật số chỗ. Gói không giới hạn không tự cộng một lượt vào số dư null. Lưu audit và yêu cầu thông báo; gửi sau commit theo ARCH-NOTIFY-01. Không xóa đăng ký, không chuyển sang buổi mới; không tạo No-show cho buổi cũ. Nếu có Attendance sớm, xử lý theo CS-07 trước khi code; không tự xóa bản ghi.

Thông báo theo BR-33 nêu rõ đăng ký đã hủy, kết quả hoàn lượt, buổi thay thế nếu có và cần tự đăng ký lại. Khả năng dùng lượt hoàn trên gói Expired chờ MEM-01.

## 4. API đề xuất cho 3 flow bắt buộc
...
| GET | `/api/users/me` | Tất cả vai trò đã xác thực | JWT |
>>>>>>> Stashed changes
| PUT | `/api/users/me` | Member/Coach/Receptionist | JWT — chỉ sửa hồ sơ của chính mình |
| POST | `/api/users/staff` | **SystemAdministrator only** | Tạo Administrator/Manager/Coach/Receptionist theo BR-2; bootstrap Administrator đầu tiên khi triển khai |
| PUT | `/api/users/{userId}/role` | **SystemAdministrator only** | Gán/đổi vai trò BR-2; bảo vệ thu hồi vai trò Administrator cuối cùng chờ AUTH-01 |
| PUT | `/api/users/{userId}/status` | **SystemAdministrator only** | Khóa/mở khóa; cấm tự khóa và khóa Administrator hoạt động cuối cùng; ngăn phiên hiện có truy cập; audit lý do BR-6/7 |
...
| PUT | `/api/sessions/{sessionId}` | **Manager only** | BR-54: buổi chưa bắt đầu; hủy đăng ký/hoàn lượt; dời tạo buổi thay thế; không giữ chỗ |
...
| DELETE | `/api/enrollments/{enrollmentId}` | Member (chủ sở hữu) hoặc Receptionist | kiểm tra ownership; hủy logic, giữ lịch sử; áp chính sách BR-50 của đăng ký; không tạo No-show |
| GET | `/api/sessions/{sessionId}/roster` | Coach (lớp mình dạy)/Manager | |
| POST | `/api/sessions/{sessionId}/check-in` | Coach/Receptionist | Coach được phân công/Receptionist ghi Present hoặc Absent; tối đa 1 bản ghi/member/session; BR-21/22/53 |
...
| POST | `/api/invoices/{invoiceId}/payments` | Receptionist | ghi nhận tiền thực nhận; BR-30/55, cọc đầu tiên cập nhật hạn đúng một lần; quá hạn và điều chỉnh chờ PAY-01/02 |
...
| POST | `/api/reports/export` | **Manager only** | NFR-PERF-REPORT-01 (PDF ≤20 trang/15s); FR-REPORT-RETRY-01 (thất bại và tạo lại) |
...
| Nhóm chức năng | Administrator | Manager | Coach | Member | Receptionist |
|---|:---:|:---:|:---:|:---:|:---:|
| Tạo tài khoản đặc quyền / gán vai trò | ✅ | ❌ | ❌ | ❌ | ❌ |
| Khóa / mở khóa user | ✅ | ❌ | ❌ | ❌ | ❌ |
| CRUD gói, bộ môn, phòng tập | ❌ | ✅ | 👁 theo API | 👁 catalog | 👁 catalog |
| Mua MemberPackage | ❌ | 👁 | ❌ | ✅ self | ✅ on-behalf |
| CRUD Class / Recurrence | ❌ | ✅ | 👁 lớp mình | 👁 | 👁 |
| Hủy / dời ClassSession | ❌ | ✅ | ❌ | ❌ | ❌ |
| Đăng ký / hủy đăng ký | ❌ | 👁 | ❌ | ✅ self | ✅ on-behalf |
| Điểm danh Present / Absent | ❌ | 👁 | ✅ buổi mình | ❌ | ✅ |
| Tạo Workout Plan / Result | ❌ | 👁 | ✅ đúng quan hệ | 👁 của mình | ❌ |
| Tạo Invoice ad-hoc / ghi Payment | ❌ | 👁 | ❌ | ❌ | ✅ |
| Tạo Adjustment | ❌ | 👁 | ❌ | ❌ | ✅ request |
| Duyệt Adjustment | ❌ | ✅ | ❌ | ❌ | ❌ |
| Xem / xuất báo cáo doanh thu | ❌ | ✅ | ❌ | ❌ | ❌ |
| Xem Audit Log | Chờ AUTH-01 | ✅ | ❌ | ❌ | ❌ |

👁 = chỉ xem theo phạm vi endpoint; ✅ = hành động; ❌ = không được cấp trong nhóm chức năng. Catalog public và hồ sơ cá nhân theo §4. Quyền quản trị tài khoản không mặc nhiên cấp quyền nghiệp vụ Manager. Việc tạo Invoice khi Member mua gói là tự động theo BR-30, không bị hàng Invoice ad-hoc giới hạn. Audit BR-7 bao gồm Administrator; ghi audit không đồng nghĩa được đọc toàn bộ audit.
...
| RabbitMQ (Notification) | Queue bất đồng bộ | **Bỏ ở MVP** — dùng background job (Hangfire/Quartz.NET) + bảng outbox trong cùng transaction nghiệp vụ, theo ARCH-NOTIFY-01 bên dưới, đáp ứng NFR-NOTIFY-01 | Khi khối lượng notification lớn hoặc cần retry phức tạp |
...
### ARCH-NOTIFY-01 — Xử lý gửi thông báo bất đồng bộ

Thiết kế phục vụ NFR-NOTIFY-01: ghi yêu cầu thông báo vào outbox trong cùng transaction nghiệp vụ; background job xử lý gửi sau khi transaction commit. Lỗi gửi không rollback hành động gốc. RabbitMQ chưa bắt buộc ở MVP; chỉ cân nhắc khi tải hoặc retry cần mở rộng. Hangfire/Quartz.NET trong bảng là lựa chọn triển khai, không phải yêu cầu nghiệp vụ.

Thiết kế này không tự chốt bảng/entity outbox, số lần retry hoặc kênh gửi thật; phải tuân thủ scope và quy trình chốt entity tại SSOT §1–2. Sự kiện cần thông báo tiếp tục theo BR-33.
...
- [ ] Xác nhận PDF report dùng thư viện nào (ảnh hưởng NFR-PERF-REPORT-01: 20 trang / 15 giây; điều kiện đo cần chốt)
