...
export default function SystemSettingsPage() {
  ...
  return (
    <AppShell
      ...
      description="Các tham số vận hành do Quản lý Trung tâm quyết định (BR-39)"
      allow={["CenterManager"]}
    >
      <Card>
        <div className="alert alert--info">
          Thay đổi cấu hình <strong>không hồi tố</strong>: chính sách hạn hủy được ghi lại vào
          từng đăng ký ngay khi hội viên đặt chỗ, nên các đăng ký đã tạo vẫn áp dụng giá trị cũ
          (BR-50).
        </div>
      </Card>

      <Card title="Tham số" bodyless>
        ...
        <AsyncSection
          state={settings}
          emptyMessage="Chưa có tham số nào."
          isEmpty={(data) => data.length === 0}
        >
          {(data) => (
            <Table headers={["Tham số", "Giá trị", "Ý nghĩa", "Cập nhật", ""]}>
              {data.map((setting) => {
